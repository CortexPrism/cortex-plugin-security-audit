...
Deno.test("audit_sast - rejects unsupported language", async () => {
  const tool = findTool("audit_sast");
  const result = await tool!.execute(
    { target_path: "/src", language: "cobol" },
    mockContext,
  );
  assertEquals(result.success, false);
...
Deno.test("audit_sast - succeeds for supported languages", async () => {
  const tool = findTool("audit_sast");
  for (
    const lang of ["javascript", "typescript", "python", "java", "go", "php"]
  ) {
    const result = await tool!.execute(
      { target_path: "/src", language: lang },
      mockContext,
    );
...
Deno.test("tool results include durationMs", async () => {
  for (const tool of tools) {
    ...
    assertExists(
      result.durationMs,
      `${tool.definition.name} missing durationMs`,
    );
