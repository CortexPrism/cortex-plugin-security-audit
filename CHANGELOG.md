...
- `audit_dependencies` — Scan project dependencies for known vulnerabilities
  (npm, pip, cargo, gomod, auto-detect)
- `audit_secrets` — Detect hardcoded secrets, API keys, tokens, passwords,
  private keys, and connection strings (30+ patterns across 10+ categories)
- `audit_sast` — Static analysis security testing for SQL injection, XSS,
  command injection, path traversal, SSRF, insecure deserialization, hardcoded
  crypto
- `audit_owasp` — OWASP Top 10 (2021/2017) compliance checking
- `audit_generate_report` — Comprehensive security audit report generation with
  severity scoring and remediation guidance (markdown, json, html)
...
- Comprehensive secret detection patterns (API keys, tokens, passwords, private
